...
export const generateRandomDiscovery = (id) => {
  ...
  if (isTokyoArea) {
    ...
    randomLat = lat.min + Math.random() * (lat.max - lat.min);
    randomLng = lng.min + Math.random() * (lng.max - lng.min);
    areaName = HAMAMATSU_AREAS[Math.floor(Math.random() * HAMAMATSU_AREAS.length)];
  }
  
  return {
    id: `d${id}`,
    name: `${areaName}${LOCATION_SUFFIXES[Math.floor(Math.random() * LOCATION_SUFFIXES.length)]}`,
    lat: randomLat,
    lng: randomLng,
 ...
 */
export const generateAmbulanceForDiscovery = (discoveryPoint, availableAmbulances, hospitals) => {
  ...
  const nearest = idleAmbulances.reduce((nearest, amb) => {
  ...
  const nearestHospital = hospitals.reduce((nearest, h) => {
...
export const createRoute = (discoveryId, ambulanceId, hospitalId, routeId) => {
  const priority = ROUTE_PRIORITIES[Math.floor(Math.random() * ROUTE_PRIORITIES.length)];
 ...
 */
export const moveAmbulanceAlongRoute = (ambulance, discovery, hospital, speed = 1, onArriveAtHospital) => {
...
const updateIdlePatrol = (ambulance, speed) => {
  ...
  if (!ambulance.patrolTargetLat || !ambulance.patrolTargetLng) {
    ...
    return {
      ...ambulance,
      patrolTargetLat: ambulance.homeHospitalLat + Math.cos(randomAngle) * randomDist,
      patrolTargetLng: ambulance.homeHospitalLng + Math.sin(randomAngle) * randomDist
    };
  }
